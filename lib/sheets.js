    ...
    requestBody: {
      values: [[
        ...
        order.paymentMethod === 'card' ? 'Carte en ligne'
          : order.paymentMethod === 'tpe' ? 'CB TPE'
          : 'Espèces',
...
async function markPaid(orderNumber) {
  ...
  if (rowIndex === -1) { console.log(`[sheets] ${orderNumber} non trouvée`); return; }

  // Colonne K = statut (11e colonne)
  await sheets.spreadsheets.values.update({
    spreadsheetId:    SHEET_ID,
    range:            `${SHEET_TAB}!K${rowIndex + 1}`,
    valueInputOption: 'RAW',
    requestBody:      { values: [['Payé']] },
  });

  console.log(`[sheets] ${orderNumber} marquée Payé`);
}
